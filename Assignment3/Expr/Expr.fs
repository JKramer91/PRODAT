...
let s2 = scomp e2 []
let s3 = scomp e3 []
printfn "this is s1 %A" (s1)
printfn "this is s2 %A" (s2)
printfn "this is s3 %A" (s3)

let compString (s: string) : sinstr list =
    let e = fromString s
    scomp e []

let exampleString = "5 + 3 * 2 - 1"
let Test1 = compString (exampleString)
printfn "Testing  %A" (Test1)


let env = [ ("a", 3); ("c", 78); ("baf", 666); ("b", 111) ]
let testEnv = If(CstI 2, CstI 11, CstI 22)

printfn "Test eval %A" (eval (fromString ("if 3 then 0 else 1")) [])
